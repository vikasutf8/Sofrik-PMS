...
const registerRider = async (req, res,next) => {
    ...
    if(isRiderExist) {
        return res.status(400).json({message: "Rider already Exist with this email"});
    }
...
const loginRider = async (req, res,next) => {
    ...
    if(!rider) {
        return res.status(400).json({message: "Rider not Exist ! Please register first"});
    }

    const isPasswordValid = await riderService.comparePassword(password, rider.password);
    if(!isPasswordValid) {
...
module.exports = {
    registerRider,loginRider,getRiderProfile,logoutRider
}
